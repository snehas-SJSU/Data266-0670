...
I set SID4 / SEED, the five words, the 10 movie titles (I wanted Batman and Dune to collide), the five RAG questions, and the writeup in this report. I ran `embeddings.ipynb`, `rag.ipynb`, and `optimizations.ipynb` on kernel Python (HW1). Mixed precision ran separately on Colab Tesla T4 (`amp_cuda_colab.ipynb`). I copied the printed tables into METRICS.md myself.

I used an assistant for gensim / LangChain boilerplate and for the Part 3 train loop (checkpoint, accum, amp). I still had to fix what it wrote. The neighbor tables, cosine numbers, RAG Yes/No ranks, MPS timing, and the CUDA AMP table are from my runs, not from it guessing.
...
For IMDB I check both extract folders. For the LLM I call Flan-T5 `generate()` instead of the removed pipeline task. Those three cells then ran and produced the tables in the report.
